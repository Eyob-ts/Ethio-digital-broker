...
export const createListing = async (req, res) => {
  try {
    ...
    if (
      category === "Car" &&
      (!condition || !model || !year || !fuel || !transmission)
    ) {
      return res
        .status(400)
    ...
    if (
      category === "House" &&
      (!price || !location || !type || !status)
    ) {
      return res
        .status(400)
        .json({ error: "House-specific fields are missing." });
    }
    //todo if (category === "Car" && (!condition || !model || !year || !fuel || !transmission)) {
    //todo   return res.status(400).json({ error: "Car-specific fields are missing" });
    //todo } else if (category === "House" && (!price || !location || !type || !status)) {
    //todo   return res.status(400).json({ error: "House-specific fields are missing" });
    // }

    // Upload image to Cloudinary
    const uploadedImages = [];
    const files = req.files?.images || []; // Multer should parse this as an array
    ...
    //todo  let cloudinaryResult;
    //todo try {
    //todo   cloudinaryResult = await cloudinary.uploader.upload(images, { folder: "listing" });
    //todo } catch (error) {
    //todo   return res.status(500).json({ error: "Image upload failed" });
    //todo }

    // Create listing
    const ownerId = req.user.id; // Assuming you have a middleware that sets `req.user`
    const newListing = new Listing({
      ...
      price: category === "House" ? price : undefined,
...
// todo    const ownerId = req.user.id;
// todo    const newListing = new Listing({
// todo      category,
//       title,
//       description,
//       price,
...
//   } catch (err) {
//     console.error("Error creating listing:", err);
//todo     res.status(500).json({ error: "Internal server error" });
//todo   }
//todo };
